...
public interface ChatMessageFacade {
    ...
    int insertRelation(String userId, String registerId, int postIdx);

    //    채팅방 들어갈 때 안읽음 -> 읽음 변경
    void updateReadYn(int relationIdx, String userIdx);

    //    게시글에서 메세지 보낼 때 관계가 존재하는 지
    Relation isRelationExist(String userId, String registerId, int postIdx);
